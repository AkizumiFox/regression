...
Let \( \Y\sim\Normal_n(\bm\theta,\sigma^2\I) \) with \( \bm\theta\in\C(\Z) \), that is, \( \E(y_{ij})=\mu_i \) depends only on the row of
\( \X \); assume \( r<c<n \). Then:
...
3. \( \text{SSLF}/\sigma^2\sim\chi^2(c-r,\gamma) \) with
   \[
\gamma=\frac{\norm{(\I-\M)\bm\theta}^2}{\sigma^2}=\frac1{\sigma^2}\sum_{i=1}^cn_i\bigl(\mu_i-\tilde\mu_i\bigr)^2,
\]{#eq-cor-lof-noncentrality}

   where \( \tilde\mu_i \) is the \( i \)th group value of \( \M\bm\theta \), the least squares fit to the true means;

4. the ratio
   \[
F=\frac{\text{SSLF}/(c-r)}{\text{SSPE}/(n-c)}\sim F(c-r,n-c,\gamma),
\]
   and \( \gamma=0 \) iff \( \bm\theta\in\C(\X) \), that is, iff the model is correct. The test that rejects the model
...
noncentralities \( \norm{(\M_Z-\M)\bm\theta}^2/\sigma^2 \) and \( \norm{(\I-\M_Z)\bm\theta}^2/\sigma^2 \). The second is zero because
\( \bm\theta\in\C(\Z) \), which proves (b). In the first, \( \M_Z\bm\theta=\bm\theta \), so \( (\M_Z-\M)\bm\theta=(\I-\M)\bm\theta \), whose
entries are \( \mu_i-\tilde\mu_i \), repeated \( n_i \) times; this is (c). Part (d) follows from @def-qf-noncentral-f,
the fact that \( (\I-\M)\bm\theta=\bzero \) iff \( \bm\theta\in\C(\X) \), and @thm-qf-f-power.
:::
...
The theorem assumes \( \bm\theta\in\C(\Z) \): the mean depends on the observation only through its row of
...
\( \gamma=m\beta_2^2\norm{(\I-\M_4)\bm q}^2/\sigma^2 \), where \( \bm q=(0,1,4,9)\T \) and \( \M_4 \) is the projection for a line
