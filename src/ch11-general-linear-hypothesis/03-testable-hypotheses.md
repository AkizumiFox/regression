...
\[
H:\ \bLambda\T\bbeta=\bm d ,
\]
with a known \( p\times m \) matrix \( \bLambda \) and a known \( \bm d\in\Real^m \). This is the
...
Let \( \bLambda \) be a \( p\times m \) matrix and \( \bm d\in\Real^m \). The hypothesis
\( H:\bLambda\T\bbeta=\bm d \) is **testable** if
...
2. the equations \( \bLambda\T\bb=\bm d \) are consistent, that is, \( \bm d\in\C(\bLambda\T) \)
   (@thm-mat-consistency).
:::
...
\[
\text{SS}_H=(\bLambda\T\hbeta-\bm d)\T\,\W\ginv\,(\bLambda\T\hbeta-\bm d),
\qquad
...
2. Let \( \bb_0 \) be any solution of \( \bLambda\T\bb_0=\bm d \) and
   ...
   \[
\text{SS}_H=\min_{\bLambda\T\bb=\bm d}\norm{\y-\X\bb}^2-\text{SSE}
   =\norm{(\M-\bP_{\mathcal S_0})(\y-\X\bb_0)}^2 .
\]

3. \( F_H\sim F(q,\,n-r,\,\gamma) \) with
   \[
\gamma=\frac{(\bLambda\T\bbeta-\bm d)\T\,\W\ginv\,(\bLambda\T\bbeta-\bm d)}{\sigma^2},
\]{#eq-glh-general-noncentrality}

   and \( \gamma=0 \) iff \( \bLambda\T\bbeta=\bm d \). In particular \( F_H \) is the \( F \) statistic of
...
\( \bLambda=\bLambda_1\bm K \) for a \( q\times m \) matrix \( \bm K \), which has rank \( q \) because
\( \rank\bLambda=q \). Hence \( \bm K\bm K\T \) is nonsingular. Put \( \bm d_1=\bLambda_1\T\bb_0 \), so that
\( \bm d=\bm K\T\bm d_1 \). Since \( \bm K\T \) has full column rank it is one-to-one, so \( \bm d_1 \) is
determined by \( \bm d \) and does not depend on the choice of \( \bb_0 \). Moreover
\( \bLambda\T\bb=\bm d \iff \bm K\T(\bLambda_1\T\bb-\bm d_1)=\bzero\iff\bLambda_1\T\bb=\bm d_1 \). The same
argument with \( \bm d \) replaced by \( \bzero \) shows that \( \mathcal S_0 \) is also
\( \{\X\bb:\bLambda_1\T\bb=\bzero\} \). The columns of \( \bLambda_1 \) are columns of \( \bLambda \), so
\( \bLambda_1\T\bbeta \) is estimable.

*(a).* By @thm-ss-hypothesis(a), \( \bu_1=\bLambda_1\T\hbeta-\bm d_1 \) and the positive definite matrix
\( \W_1=\bLambda_1\T\G\bLambda_1 \) do not depend on \( \hbeta \) and \( \G \). Now
\( \bu=\bLambda\T\hbeta-\bm d=\bm K\T\bu_1 \) and \( \W=\bm K\T\W_1\bm K \). Let
\( \bv=\bm K\T(\bm K\bm K\T)^{-1}\W_1^{-1}\bu_1 \), so that \( \bm K\bv=\W_1^{-1}\bu_1 \). Then
\( \W\bv=\bm K\T\W_1\bm K\bv=\bm K\T\bu_1=\bu \), and for every generalized inverse,
\[
\bu\T\W\ginv\bu=\bv\T\W\W\ginv\W\bv=\bv\T\W\bv=(\bm K\bv)\T\W_1(\bm K\bv)=\bu_1\T\W_1^{-1}\bu_1 .
\]
...
because \( \C(\bLambda)\subseteq\C(\X\T) \). The solutions of \( \bLambda\T\bb=\bm d \) are \( \bb_0+\Null(\bLambda\T) \),
...
\( \bm d_1 \), the same difference equals \( \bu_1\T\W_1^{-1}\bu_1 \), which is \( \text{SS}_H \) by (a).
...
squares estimate: \( \sigma^2\gamma=(\bLambda\T\bbeta-\bm d)\T\W\ginv(\bLambda\T\bbeta-\bm d) \). Finally, by (a)
this equals \( \bu_1(\bbeta)\T\W_1^{-1}\bu_1(\bbeta) \) with \( \bu_1(\bbeta)=\bLambda_1\T\bbeta-\bm d_1 \). It is zero
iff \( \bLambda_1\T\bbeta=\bm d_1 \), that is, iff \( \bLambda\T\bbeta=\bm d \).
:::

Formula @eq-glh-general-F needs only the unrestricted fit. Its ingredients have a statistical
meaning: \( \bLambda\T\hbeta-\bm d \) is the estimated departure from the hypothesis, and
...
Let \( H:\bLambda\T\bbeta=\bm d \) be testable with \( \bLambda \) of full column rank \( q \), and write
\( \bLambda=\X\T\bT \). Let \( \G \) be a generalized inverse of \( \X\T\X \), \( \hbeta=\G\X\T\y \),
\( \W=\bLambda\T\G\bLambda \) and \( \bu=\bLambda\T\hbeta-\bm d \). Then \( \W \) is positive definite, and:

::: {.enumerate options="label=(\alph*)"}
1. \( \hbeta_H=\hbeta-\G\bLambda\W^{-1}\bu \) satisfies \( \bLambda\T\hbeta_H=\bm d \) and minimizes
   \( \norm{\y-\X\bb}^2 \) subject to \( \bLambda\T\bb=\bm d \). The set of all minimizers is
...
3. With \( \hat{\bm\theta}=\W^{-1}\bu \), the restricted estimate solves the constrained normal equations (compare @thm-proj-normal-equations)
   \[
\X\T\X\hbeta_H+\bLambda\hat{\bm\theta}=\X\T\y,\qquad \bLambda\T\hbeta_H=\bm d,
\]

   and \( \text{SS}_H=\hat{\bm\theta}\T\W\hat{\bm\theta} \).
:::
...
(a) \( \bLambda\T\hbeta_H=\bLambda\T\hbeta-\W\W^{-1}\bu=\bm d \). For any feasible \( \bb \), @eq-proj-distance-split with
...
(c) \( \X\T\X\hbeta_H=\X\T\X\hbeta-\X\T\X\G\X\T\bT\W^{-1}\bu=\X\T\y-\X\T\M\bT\hat{\bm\theta}=\X\T\y-\bLambda\hat{\bm\theta} \),
because \( \X\T\X\hbeta=\X\T\y \) and \( \X\T\M=\X\T \). The last identity is
\( \bu\T\W^{-1}\bu=\hat{\bm\theta}\T\W\hat{\bm\theta} \).
:::

Part (c) identifies \( \hat{\bm\theta} \) as a vector of Lagrange multipliers (compare @prp-mat-lagrange).
...
\( \bb_0 \) of \( \bLambda\T\bb_0=\bm d \) may be used. Two solutions differ by an element of \( \Null(\bLambda\T) \),
so their offsets differ by an element of \( \mathcal S_0 \), which the reduced model absorbs. In practice
one fits \( \y-\X\bb_0 \) on a basis of \( \mathcal S_0 \), for example the columns of \( \X\bm U \) where the columns of
\( \bm U \) span \( \Null(\bLambda\T) \), and takes the difference of residual sums of squares. The formula
...
Let \( \bLambda \) be \( p\times m \) and \( \bm A \) a nonsingular \( m\times m \) matrix. Show that the hypotheses \( \bLambda\T\bbeta=\bm d \) and
\( (\bLambda\bm A)\T\bbeta=\bm A\T\bm d \) are the same, and that @eq-glh-general-F gives the same \( \text{SS}_H \) for
both. Conclude that the test does not depend on how a set of constraints is written.
:::

::: {.solution}
\( (\bLambda\bm A)\T\bb=\bm A\T\bm d\iff\bm A\T(\bLambda\T\bb-\bm d)=\bzero\iff\bLambda\T\bb=\bm d \), since \( \bm A\T \)
is nonsingular. So the constraint sets, and by @thm-glh-general-f(b) the sums of squares, agree.
Directly: with \( \bu=\bLambda\T\hbeta-\bm d \), the new departure is \( \bm A\T\bu \) and the new middle matrix is
\( \bm A\T\W\bm A \), and one generalized inverse of it is \( \bm A^{-1}\W\ginv\bm A^{-\top} \), which gives
\( \bu\T\bm A\bm A^{-1}\W\ginv\bm A^{-\top}\bm A\T\bu=\bu\T\W\ginv\bu \). The choice of generalized inverse does not
...
In @prp-glh-restricted-ls, show that \( \E(\X\hbeta_H)=\X\bbeta-\M\bT\W^{-1}(\bLambda\T\bbeta-\bm d) \) and
...
::: {.solution}
\( \X\hbeta_H=\M\Y-\M\bT\W^{-1}(\bT\T\M\Y-\bm d) \), using \( \bLambda\T\hbeta=\bT\T\M\Y \). Its mean is
\( \X\bbeta-\M\bT\W^{-1}(\bT\T\X\bbeta-\bm d) \), and \( \bT\T\X\bbeta=\bLambda\T\bbeta \). The random part is
...
nonnegative definite. The bias is \( -\M\bT\W^{-1}(\bLambda\T\bbeta-\bm d) \), zero iff \( H \) holds, because
