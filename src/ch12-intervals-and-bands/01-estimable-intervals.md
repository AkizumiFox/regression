...
::: {.proof}
(a) Write \( \blambda=\X\T\bm\rho \). Then \( \blambda\T\hbeta=\bm\rho\T\X\hbeta=\bm\rho\T\M\Y \) and
\( \blambda\T\G\blambda=\bm\rho\T\X\G\X\T\bm\rho=\bm\rho\T\M\bm\rho \) by @thm-proj-M-formula. Neither
...
In general \( \blambda\T\G\blambda=\norm{\M\bm\rho}^2 \), with \( \blambda=\X\T\bm\rho \), is the smallest squared length of a
vector \( \bm a \) with \( \E(\bm a\T\Y)=\blambda\T\bbeta \) (@thm-opt-gauss-markov).
