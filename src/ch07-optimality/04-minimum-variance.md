...
\( \Y\T\Y=\text{SSE}+\norm{\M\Y}^2 \). Also \( \X\T\Y=\X\T\Q(\Q\T\Y) \), and \( \Q=\X\mathbf{K} \) for some \( \mathbf{K} \), since the
columns of \( \Q \) lie in \( \C(\X) \), so \( \Q\T\Y=\mathbf{K}\T\X\T\Y \). A one-to-one function of a complete sufficient
...
Let \( \mathbf{a}\T\Y \) be any LUE of an estimable \( \blambda\T\bbeta \), such as the end-point slope of @exm-opt-three-slopes.
...
So the pairs \( (\mathbf{a}\T\Y,\bT(\Y)) \) and \( (\mathbf{a}\T\R\Y,\bT(\Y)) \) have the same joint distribution, where
\( \bT=(\X\T\Y,\Y\T\Y) \). Hence \( \E(\mathbf{a}\T\Y\mid\bT)=\E(\mathbf{a}\T\R\Y\mid\bT) \), and averaging,
\[
\E(\mathbf{a}\T\Y\mid\bT)=\E\Bigl(\mathbf{a}\T\frac{\Y+\R\Y}2\,\Big|\,\bT\Bigr)=\E(\mathbf{a}\T\M\Y\mid\bT)=\mathbf{a}\T\M\Y=\blambda\T\hbeta ,
\]
because \( \mathbf{a}\T\M\Y \) is a function of \( \bT \) and \( \M\mathbf{a}=\mathbf{a}_* \) (@prp-opt-lue-set). The Rao–Blackwell
improvement of *every* LUE is the least squares estimate. The listing checks the identity
\( \tfrac12(\mathbf{a}\T\y+\mathbf{a}\T\R\y)=\blambda\T\hbeta \) for simulated data and the end-point slope.
:::
...
@thm-qf-mean-var(c) with \( \mathbf{K}=\X\T \), \( \A=\I \) and \( \bmu=\bzero \), \( \Cov(\X\T\be,\be\T\be)=\bzero \).
:::
...
\end{aligned}
\]
where \( \mathbf{g}_{\bbeta}=\partial g/\partial\bbeta \) and \( g_v=\partial g/\partial v \).
:::
...
\end{aligned}
\]
the last step because \( \E\mathbf{S}=\bzero \). For any constant vector \( \mathbf{c} \),
\( 0\le\Var(\delta-\mathbf{c}\T\mathbf{S})=\Var\delta-2\mathbf{c}\T\nabla g+\mathbf{c}\T\boldsymbol{\mathcal I}\mathbf{c} \). Choosing
\( \mathbf{c}=\boldsymbol{\mathcal I}^{-1}\nabla g \) gives \( \Var\delta\ge\nabla g\T\boldsymbol{\mathcal I}^{-1}\nabla g \). The block-diagonal
...
\( \M\Y \) by @thm-mvn-independence, since \( \Cov(\M\Y,\Z)=\sigma^2\M\Q_2=\mathbf{0} \). Then
\( \text{SSE}=\norm{\Z}^2 \) and \( \bU=\Q_2\Z/\norm{\Z} \). For any orthogonal \( \mathbf{H} \), \( \mathbf{H}\Z \) has the same law as \( \Z \), so the
...
(a) Show that \( \A\X=\mathbf{0} \) and \( \tr\A=1 \). (b) Using @thm-rv-quadform-variance, show that
