...
\( \X=[\bone,\bm d,\bone-\bm d] \) has three columns, but they satisfy
\( \bone-\bm d-(\bone-\bm d)=\bzero \), so \( \rank(\X)=2 \). This is the **dummy variable trap**: an
...
A **coding** of a factor with \( g \) levels is a nonsingular \( g\times g \) matrix \( \bm K \). The coded model
matrix is \( \Z\bm K \), and the model is \( \E(\Y)=\Z\bm K\bgamma \). When the first column of \( \bm K \) is \( \bone_g \),
so that \( \bm K=[\bone_g,\bm C] \), the coded matrix is \( [\bone_n,\Z\bm C] \): an intercept and the \( g-1 \) columns
\( \Z\bm C \). The \( g\times(g-1) \) matrix \( \bm C \) is then called the **coding matrix** of the factor.
:::

Row \( k \) of \( \bm K \) lists the values that the coded columns take for an observation at level \( k \). In the
software of [Section 8.4](04-side-conditions.html) the matrix \( \bm C \) is what one specifies (in R, the
“contrasts” attribute of a factor), and the program builds the columns \( \Z\bm C \) from it.

::: {#prp-est-coding}
[What a coding estimates]

Let \( \bm K \) be a coding and \( \hat{\bmu}=\bD^{-1}\Z\T\y \) the vector of level means. Then:

::: {.enumerate options="label=(\alph*)"}
1. \( \C(\Z\bm K)=\C(\Z) \), so every coding has the same fitted values, residuals and residual sum of squares;

2. the coefficients are \( \bgamma=\bm K^{-1}\bmu \), and their least squares estimates are
           \( \hat{\bgamma}=\bm K^{-1}\hat{\bmu} \);

3. \( [\bone_g,\bm C] \) is nonsingular iff the columns of \( \bm C \) are linearly independent and
           \( \bone_g\notin\C(\bm C) \);

4. to make the coefficients estimate prescribed linear functions \( \bm L\bmu \) of the level means, with \( \bm L \)
           a nonsingular \( g\times g \) matrix, use the coding \( \bm K=\bm L^{-1} \).
:::

:::

::: {.proof}
(a) The columns of \( \Z\bm K \) lie in \( \C(\Z) \), and \( \rank(\Z\bm K)=\rank(\Z)=g \) because \( \bm K \) is nonsingular
...
(b) The two models describe the same means iff \( \Z\bm K\bgamma=\Z\bmu \), and since \( \Z \) has full column rank this
means \( \bm K\bgamma=\bmu \). For the estimates, \( \Z\bm K\hat{\bgamma}=\M\y=\Z\hat{\bmu} \), and the same cancellation
gives \( \bm K\hat{\bgamma}=\hat{\bmu} \).
(c) A square matrix is nonsingular iff its columns are linearly independent. The columns \( \bone_g,\bm c_1,\dots,\bm c_{g-1} \) are
independent iff the \( \bm c_j \) are independent and \( \bone_g \) is not in their span.
(d) With \( \bm K=\bm L^{-1} \), (b) gives \( \bgamma=\bm L\bmu \).
:::

Part (b) is the whole story. Whatever the coding, the coefficients are fixed linear combinations of the
level means, and these combinations are the *rows of \( \bm K^{-1} \)*, not the columns of \( \bm K \). Each such
...
answer, write them as the rows of \( \bm L \), and invert.
...
the intercept \( \gamma_0 \) and of the other coefficients \( \gamma_1,\dots,\gamma_{g-1} \), that is, the rows of \( \bm K^{-1} \).

- **Reference (treatment) coding.** \( \bm C=\begin{psmallmatrix}\bzero\T\\\I_{g-1}\end{psmallmatrix} \): the columns are the
...
- **Sum-to-zero (deviation or effect) coding.** \( \bm C=\begin{psmallmatrix}\I_{g-1}\\-\bone\T\end{psmallmatrix} \). Level \( k<g \) has
...
- **Weighted effect coding.** \( \bm C=\begin{psmallmatrix}\I_{g-1}\\-(n_1,\dots,n_{g-1})/n_g\end{psmallmatrix} \). Now
...
- **Successive differences.** With \( \bm L \) having first row \( g^{-1}\bone\T \) and rows \( \bm e_{j+1}\T-\bm e_j\T \), part (d) gives
...
\( \bm C \) in the order defined above; for the two deviation codings they belong to the first six levels):
...
Each row is \( \bm K^{-1}\hat{\bmu} \) for its own \( \bm K \), as @prp-est-coding(b) says, and all four have residual sum of
...
\( \bm K^{-1}\hat{\bmu} \). To get the comparisons you want, write them down first as the rows of a
nonsingular matrix \( \bm L \), then code with \( \bm K=\bm L^{-1} \).
:::

For an ordered factor, a useful instance of this principle is the *cumulative* coding, with columns
\( \bm 1\{\text{level}\ge k\} \) for \( k=2,\dots,g \). Then \( \bm K \) is lower triangular with ones on and below the diagonal. Its
...
(level 2), write down the coding matrix \( \bm K \) with \( \bmu=\bm K\bbeta \). Use @prp-est-coding(b), not an inverse
...
\( \bm K=\begin{psmallmatrix}1&0\\1&1\end{psmallmatrix} \) and
\( \bm K^{-1}=\begin{psmallmatrix}1&0\\-1&1\end{psmallmatrix} \). By @prp-est-coding(b),
\( \hbeta=\bm K^{-1}\hat{\bmu}=(\bar{y}_0,\,\bar{y}_1-\bar{y}_0)\T \). The level means are the estimates of the full-rank
...
Let \( \X \) have full column rank, let \( \bm e_i \) be the \( i \)th coordinate vector with \( \bm e_i\notin\C(\X) \), and
fit \( [\X,\bm e_i] \) with coefficients \( (\bbeta,\delta) \). Write \( \hat{e}_i \) and \( h_{ii} \) for the residual and
the leverage (@def-proj-leverage) of observation \( i \) in the fit of \( \X \) alone.

::: {.enumerate options="label=(\alph*)"}
1. Use @thm-proj-fwl, with \( \bm e_i \) as the regressor of interest and \( \X \) as the one partialled out, to
...
::: {.solution}
(a) Apply @thm-proj-fwl(a) with \( \X_1=\bm e_i \) and \( \X_2=\X \), and let \( \M \) project onto \( \C(\X) \). Then
\( \hat{\delta} \) is the coefficient in the regression of \( \y \) on \( (\I-\M)\bm e_i \):
\( \hat{\delta}=\bm e_i\T(\I-\M)\y/\bm e_i\T(\I-\M)\bm e_i=\hat{e}_i/(1-h_{ii}) \). The denominator is
\( \norm{(\I-\M)\bm e_i}^2 \), which is positive because \( \bm e_i\notin\C(\X) \).
(b) The residual vector of the enlarged fit is orthogonal to every column, in particular to \( \bm e_i \), so its
...
zero. What remains is the deletion criterion, and its minimizer \( \hbeta_{(i)} \) is unique: if \( \X\bm b=c\bm e_i \) with
\( \bm b\ne\bzero \), then \( c\ne0 \) by full rank, and \( \bm e_i\in\C(\X) \). So \( \hbeta=\hbeta_{(i)} \).
...
\( k=2,\dots,g \), together with an intercept. Show that \( \bm K \) is lower triangular with ones on and below the
diagonal, that \( \bm K^{-1} \) is the first-difference matrix, and hence that the coefficients are \( \mu_1 \) and
\( \mu_k-\mu_{k-1} \).
:::
...
identifiable iff the vectors \( (N_{1i}) \) and \( (N_{2i}) \) are not proportional. Find \( \Cov(\bm V) \); why is it not a
