...
local function scan_and_dump_labels(doc)
    ...
    do
        ...
        local function add_cites(block, kind)
            if not current then return end
            uses[current] = uses[current] or {}
            block:walk { Cite = function(cite)
                for _, citation in ipairs(cite.citations) do
                    if citation.id ~= current then
                        local kinds = uses[current][citation.id]
                        if not kinds then kinds = {}; uses[current][citation.id] = kinds end
                        kinds[kind] = true
                    end
                end
            end }
